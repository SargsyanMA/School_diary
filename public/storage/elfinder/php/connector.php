...
if (!file_exists('../files/'.User::getInstance()->getUserId().'/files')) {
	mkdir('../files/'.User::getInstance()->getUserId().'/files', 0777, true);
}
...
$opts = array(
	...
	'roots' => array(
		array(
			...
			'path'          => '../files/'.User::getInstance()->getUserId().'/files/',                 // path to files (REQUIRED)
			'URL'           => dirname($_SERVER['PHP_SELF']) . '/../files/'.User::getInstance()->getUserId().'/files/', // URL to files (REQUIRED)
