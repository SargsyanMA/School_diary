        ...
        <div class="row">
            ...
            <div class="col-md-4">
                ...
                @if(isset($scores[$item->lesson->id]))
                    @foreach($scores[$item->lesson->id] as $score)
                        <div style="float: left; text-align: center; padding: 0 4px;" title="{{ $score->name ?? '' }}, {{ $score->comment ?? '' }}" style=" padding: 0; font-size: 12px; border-top:none;">
                            <span style="font-size: 16px;">{{ $score->value ?? '' }}<sub>{{ $score->weight ?? '' }}</sub></span><br>
                            <small class="text-muted">{{ isset($score->date)?\Carbon\Carbon::parse($score->date)->format('d.m'):'' }}</small><br>
                        <!--div style="max-width: 75px;"><small class="text-muted">{{ $score->comment ?? '' }}</small></div-->
                        </div>
                    @endforeach
                @endif
            ...
            <div class="col-md-1">
                @if(isset($homeworks[$item->lesson->id]))
                    ...
                    @foreach($homeworks[$item->lesson->id] as $homework)
                        <small>{{ isset($homework->date)?Carbon::parse($homework->date)->format('d.m'):'' }}</small>
                    @endforeach
