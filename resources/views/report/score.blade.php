        ...
        <div class="filter">
            <form method="get" class="row">
                ...
                <div class="form-group col-md-3 pull-right text-right" style="padding-top: 18px;">
                    @if($student)
                        <a href="/sendmail/score/{{ $student->id }}" class="btn btn-info js-send-score" data-name="{{ $student->name }}" title="Отправить оценки"><i class="fa fa-graduation-cap" aria-hidden="true"></i></a>
                    @endif
                    <a href="{{url('/reports/score/print?').http_build_query(request()->query())}}" target="_blank" class="btn btn-info"><i class="fa fa-print"></i></a>
                    <a href="{{url('/reports/score-export?').http_build_query(request()->query())}}" target="_blank" class="btn btn-info"><i class="far fa-file-excel"></i></a>
                </div>
            </form>
        </div>
    @endif
